...
use esp_idf_hal::gpio::{Gpio18, Gpio45, Output, PinDriver};
...
static mut CS_R: Option<PinDriver<'_, Gpio45, Output>> = None;
static mut CS_L: Option<PinDriver<'_, Gpio18, Output>> = None;
...
pub fn init(peripherals: &mut Peripherals) -> anyhow::Result<()> {
    unsafe {
        ...
        let sclk = peripherals.pins.gpio17.clone_unchecked();
        let sda = peripherals.pins.gpio13.clone_unchecked();
        let sdi = peripherals.pins.gpio14.clone_unchecked();
        let cs_r = peripherals.pins.gpio45.clone_unchecked();
        let cs_l = peripherals.pins.gpio18.clone_unchecked();

...
struct ImuHardware<'a> {
    ...
    chip_select: PinDriver<'a, Gpio21, Output>,
}
...
pub fn init(peripherals: &mut Peripherals) -> anyhow::Result<()> {
    unsafe {
        ...
        let sclk = peripherals.pins.gpio20.clone_unchecked(); // SCLK
        let sdo = peripherals.pins.gpio19.clone_unchecked(); // MOSI
        let sdi = peripherals.pins.gpio18.clone_unchecked(); // MISO
        let cs = peripherals.pins.gpio21.clone_unchecked();

...
use esp_idf_hal::gpio::{Gpio10, Gpio6, Output, PinDriver};
...
struct ImuHardware<'a> {
    ...
    cs_batt: PinDriver<'a, Gpio6, Output>,
}
...
pub fn init(peripherals: &mut Peripherals) -> anyhow::Result<()> {
    unsafe {
        ...
        HARDWARE = Some(ImuHardware {
            ...
            cs_batt: PinDriver::output(cs_batt)?,
        });
    }
...
pub fn read_batt() -> anyhow::Result<u16> {
    ...
    let mut read = [0, 0];
    unsafe {
        HARDWARE.as_mut().unwrap().cs_batt.set_low()?;
    }
    FreeRtos::delay_us(1);
    unsafe {
        HARDWARE.as_mut().unwrap().spi.transfer(&mut read, &write)?;
        HARDWARE.as_mut().unwrap().cs_batt.set_high()?;
    }

...
use esp_idf_hal::gpio::{
    Gpio1, Gpio2, Gpio3, Gpio4, Gpio5, Gpio6, Gpio7, Gpio8, Output, PinDriver,
};
...
struct WallSensorHardware<'a> {
    ad: AdcDriver<'a, ADC1>,
    ls: AdcChannelDriver<'a, Gpio4, Atten11dB<ADC1>>,
    lf: AdcChannelDriver<'a, Gpio3, Atten11dB<ADC1>>,
    rf: AdcChannelDriver<'a, Gpio2, Atten11dB<ADC1>>,
    rs: AdcChannelDriver<'a, Gpio1, Atten11dB<ADC1>>,
    ena: PinDriver<'a, Gpio6, Output>,
    sel0: PinDriver<'a, Gpio7, Output>,
    sel1: PinDriver<'a, Gpio8, Output>,
...
pub fn init(peripherals: &mut Peripherals) -> anyhow::Result<()> {
    unsafe {
        ...
        let ls = AdcChannelDriver::new(peripherals.pins.gpio4.clone_unchecked())?;
        let lf = AdcChannelDriver::new(peripherals.pins.gpio3.clone_unchecked())?;
        let rf = AdcChannelDriver::new(peripherals.pins.gpio2.clone_unchecked())?;
        let rs = AdcChannelDriver::new(peripherals.pins.gpio1.clone_unchecked())?;
        let batt = AdcChannelDriver::new(peripherals.pins.gpio5.clone_unchecked())?;
        let sel0 = PinDriver::output(peripherals.pins.gpio7.clone_unchecked())?;
        let sel1 = PinDriver::output(peripherals.pins.gpio8.clone_unchecked())?;
        let ena = PinDriver::output(peripherals.pins.gpio6.clone_unchecked())?;
